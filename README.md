...
Supporting documentation: **`EDA_DOCUMENTATION.md`** (EDA and variable definitions), **`PLOTS_EXPLANATION.md`** (figure-by-figure guide), **`GIT_SPEED_TIPS.md`** (optional Git workflow notes).
...
- **Never-treated** arm: market index series with **`G = 0`**; treated firm arm gets a cohort **`G`** set so treatment turns on at the mapped period (see comments in script).  
